...
class SupportedShortcodes extends Shortcode
{   
     ...
     * @return string
     */
    public function accordeon($count, $color)
    {
        $extend = '';
        for ($i = 1; $i <= $count; $i++) {
            $extend .= '[collapse color="' . $color . '" title="' . Text::getHeadline() . '"]' . $this->getTemplateParts('text-medium') . '[/collapse]';
        };
